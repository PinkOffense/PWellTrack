...
function handleDemo() {
  auth.user = {
    id: 0, name: 'Demo', email: 'demo@pwelltrack.com',
...
<template>
  <div class="auth-page">
    <div class="auth-bg">
      <div v-for="i in 5" :key="i" :class="'paw paw-' + i">
        <PawPrint :size="20" />
      </div>
    </div>
    ...
    <div class="auth-card animate-slideUp">
      ...
      <div v-if="!auth.backendReachable && !auth.loading" class="alert-warning mb-4">
        {{ t('auth.offlineMsg') }}
      </div>
...
.paw { position: absolute; color: rgba(155,142,200,0.12); animation: float 3s ease-in-out infinite; }
.paw-1 { top: 10%; left: 8%; animation-delay: 0s; }
.paw-2 { top: 25%; right: 12%; animation-delay: 0.6s; }
.paw-3 { bottom: 30%; left: 15%; animation-delay: 1.2s; }
.paw-4 { bottom: 15%; right: 8%; animation-delay: 1.8s; }
.paw-5 { top: 50%; left: 50%; animation-delay: 2.4s; }
@keyframes float {
...
.input-icon {
  position: absolute; left: 14px; color: var(--c-txt-muted); pointer-events: none; z-index: 1;
}
...
.btn-google {
  ...
  color: var(--c-txt); background: #fff;
