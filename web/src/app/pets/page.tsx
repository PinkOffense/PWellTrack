...
import type { Pet, PetCreate, PetDashboard, VaccineStatusSummary } from '@/lib/types';
...
const VACCINE_STATUS_STYLE: Record<string, { key: string; color: string; bg: string }> = {
  no_records: { key: 'noData', color: 'text-gray-400', bg: 'bg-gray-50' },
  overdue: { key: 'overdue', color: 'text-red-500', bg: 'bg-red-50' },
  due_soon: { key: 'dueSoon', color: 'text-amber-500', bg: 'bg-amber-50' },
  up_to_date: { key: 'upToDate', color: 'text-emerald-500', bg: 'bg-emerald-50' },
};
...
export default function PetsPage() {
  ...
  const [vaccineStatuses, setVaccineStatuses] = useState<Record<number, VaccineStatusSummary>>({});
  ...
  const loadPets = async () => {
    try {
      // Single API call replaces the old 2N+1 pattern (GET /pets/ + N*today + N*vaccines)
      const summary = await petsApi.summary();
      setPets(summary.map(s => s.pet));
      const dMap: Record<number, PetDashboard | null> = {};
      const vsMap: Record<number, VaccineStatusSummary> = {};
      for (const s of summary) {
        dMap[s.pet.id] = s.dashboard;
        vsMap[s.pet.id] = s.vaccine_status;
      }
      setDashboards(dMap);
      setVaccineStatuses(vsMap);
    } catch (err: any) {
        ...
        {pets.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
            {pets.map((pet, i) => {
              ...
              const vs = vaccineStatuses[pet.id];
              const feedStatus = getFeedingStatus(dash);
              const vaxStatus = vs ? VACCINE_STATUS_STYLE[vs.status] ?? VACCINE_STATUS_STYLE.no_records : VACCINE_STATUS_STYLE.no_records;
