...
const SEV_COLORS: Record<string, string> = { mild: 'badge-green', moderate: 'badge-amber', severe: 'badge-red' };

function SymptomForm({ petId, t, onSave, editingItem }: { petId: number; t: any; onSave: () => void; editingItem?: Symptom }) {
  const [type, setType] = useState(editingItem?.type ?? '');
  const [severity, setSeverity] = useState<string>(editingItem?.severity ?? 'mild');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) await symptomsApi.update(editingItem.id, payload);
      else await symptomsApi.create(petId, payload);
      onSave();
...
export default function SymptomsPage() {
  ...
  return (
    <RecordPage<Symptom>
      ...
      updateFn={symptomsApi.update}
      supportsDateFilter
      renderItem={(item, t) => (
        <>
          <div className="flex items-center gap-2">
            <span className="font-semibold text-txt">{item.type}</span>
            <span className={SEV_COLORS[item.severity] || 'badge-gray'}>{t(`symptoms.${item.severity}` as any)}</span>
          </div>
      ...
      renderForm={({ petId, t, onSave, editingItem }) => (
        <SymptomForm key={editingItem?.id ?? 'new'} petId={petId} t={t} onSave={onSave} editingItem={editingItem} />
      )}
    />
  );
}
